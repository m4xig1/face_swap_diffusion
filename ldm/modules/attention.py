...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        if context.shape[-1] == 768 * 2:
            ...
            k = torch.cat(
                [
                    k1[:, :, : self.head_splits[0] * self.dim_head],
                    k2[:, :, -self.head_splits[1] * self.dim_head :],
                ],
                dim=-1,
            )
            v = torch.cat(
                [
                    v1[:, :, : self.head_splits[0] * self.dim_head],
                    v2[:, :, -self.head_splits[1] * self.dim_head :],
                ],
                dim=-1,
            )
            # head_splits=[6,2]
