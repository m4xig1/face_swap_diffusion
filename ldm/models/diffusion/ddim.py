        ...
        **kwargs,
        ...
        if x.shape[1] != 4:
            ...
            if src_im is not None:
                ...
                masks = 1 - TF.resize(
                    x[:, 8, :, :], (pred_x0_im.shape[2], pred_x0_im.shape[3])
                )
                # mask x_samples_ddim
                ...
                ID_loss, _, seperate_sim = self.model.face_ID_model(
                    pred_x0_im_masked, src_im, clip_img=False, return_seperate=True
                )
        ...
        if seperate_sim is None:
            ...
            x_prev = (
                a_prev.sqrt() * pred_x0
                + seperate_sim.view(-1, 1, 1, 1).to(self.model.device) * dir_xt
                + noise
            )
