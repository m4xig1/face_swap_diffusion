    ...
    def __getitem__(self, index):
        img_path = os.path.join(
            os.path.join(
                self.test_bench_dir,
                "GT_3500",
                str(self.id_list[index]).zfill(12) + "_GT.png",
            )
        )
        ...
        ref_img_path = os.path.join(
            os.path.join(
                self.test_bench_dir,
                "Ref_3500",
                str(self.id_list[index]).zfill(12) + "_ref.png",
            )
        )
        ...
        mask_path = os.path.join(
            os.path.join(
                self.test_bench_dir,
                "Mask_bbox_3500",
                str(self.id_list[index]).zfill(12) + "_mask.png",
            )
        )
        ...
        return (
            ...
            str(self.id_list[index]).zfill(12),
        )
...
class CelebAdataset(data.Dataset):
    def __init__(
        self, state, load_vis_img=False, label_transform=None, fraction=1.0, **args
    ):
        ...
        self.trans = A.Compose([A.Resize(height=224, width=224)])
        ...
        if state == "train":
            self.imgs = sorted(
                [
                    osp.join(args["dataset_dir"], "CelebA-HQ-img", "%d.jpg" % idx)
            ...
            self.labels_vis = (
                sorted(
                    [
                        osp.join(args["dataset_dir"], "vis", "%d.png" % idx)
                        for idx in range(28000)
                    ]
                )
            ...
            self.ref_labels_vis = (
                self.ref_labels_vis[: int(len(self.labels_vis) * self.fraction)]
                if self.load_vis_img
                else None
            )
        ...
        self.labels_vis = (
            self.labels_vis[: int(len(self.labels_vis) * self.fraction)]
            if self.load_vis_img
            else None
        )
    ...
    def __getitem_gray__(self, index):
        ...
        mask_tensor = 1 - get_tensor(normalize=False, toTensor=True)(mask_img)
        reference_mask_tensor = get_tensor(normalize=False, toTensor=True)(
            ref_converted_mask
        )
        inpaint_tensor = image_tensor * mask_tensor
    ...
    def __init__(
        ...
        if state == "train":
            ...
            self.imgs = sorted(
                [
                    osp.join(
                        args["dataset_dir"], "Val_target", "{0:0=4d}.png".format(idx)
                    )
            ...
            self.labels = sorted(
                [
                    osp.join(
                        args["dataset_dir"], "target_mask", "{0:0=4d}.png".format(idx)
                    )
                    for idx in range(0, 500)
                ]
            )
            ...
            self.ref_imgs = sorted(
                [
                    osp.join(args["dataset_dir"], "Val", "{0:0=4d}.png".format(idx))
                    for idx in range(500, 1000)
                ]
            )
            self.ref_labels = sorted(
                [
                    osp.join(
                        args["dataset_dir"], "src_mask", "{0:0=4d}.png".format(idx)
                    )
                    for idx in range(500, 1000)
                ]
            )
            ...
            if self.load_prior:
                self.prior_images = sorted(
                    [
                        osp.join(
                            "intermediate_results_FFHQ_261/results",
                            "0000000%d.jpg" % idx,
                        )
                        for idx in range(68000, 69000)
                    ]
