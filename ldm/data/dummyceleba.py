class CelebAHQDataset(Dataset):
    def __init__(
        ...
        label_transform=TO_TENSOR,
        load_vis_img=False,
        fraction=1.0,
        flip_p=-1,
    ):  # negative number for no flipping
        ...
        if mode == "train":
            self.imgs = sorted(
                [
                    osp.join(self.root, "CelebA-HQ-img", "%d.jpg" % idx)
            ...
            self.labels = sorted(
                [
                    osp.join(self.root, "CelebA-HQ-mask/Overall_mask", "%d.png" % idx)
                    for idx in range(28000)
                ]
            )
            self.labels_vis = (
                sorted(
                    [osp.join(self.root, "vis", "%d.png" % idx) for idx in range(28000)]
                )
            ...
            self.labels_vis = (
                sorted(
                    [
                        osp.join(self.root, "vis", "%d.png" % idx)
                        for idx in range(28000, 30000)
                    ]
                )
                if self.load_vis_img
